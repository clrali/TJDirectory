...
function SchoolCalendar() {
  ...
  useEffect(() => {
    const info = [];
    getDocs(collection(db, "Events")).then((allInfo) => {
    ...
    <div className="schoolCalendar">
        ...
        <tbody>
          <tr>
            ...
            <td>
              {info.map((event) => (
                <Data
                  property={new Date(event.date.seconds * 1000).toDateString()}
                />
              ))}
            </td>
            <td>
      ...
      <div className="eventForm">
        <form onSubmit={addEvent}>
          ...
          <Button className="eventForm" type="submit" variant="outlined">
            Add Event
          </Button>
        </form>
      </div>
